...
public final class Platform {
    ...
    public static void registerKeys(JukeboxPlus mod) {
        KeyMapping.Category category = KeyMapping.Category.register(Identifier.fromNamespaceAndPath(JukeboxPlus.MOD_ID, "main"));
        for (JukeboxPlus.Action a : JukeboxPlus.Action.values()) {
            ...
            KEYS.put(a, KeyMappingHelper.registerKeyMapping(km));
        }
    }

    public static void registerHud(JukeboxPlus mod) {
        HudElementRegistry.attachElementAfter(VanillaHudElements.SUBTITLES,
                Identifier.fromNamespaceAndPath(JukeboxPlus.MOD_ID, "now_playing"),
                (graphics, deltaTracker) -> mod.renderHud(new GfxImpl(graphics)));
    }
